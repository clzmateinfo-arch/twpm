    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-300 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-2xl max-w-md w-full">
                ...
                <h2 className="text-2xl font-black text-white text-center mb-2">TWPMS Security</h2>
                <p className="text-slate-500 text-center text-sm mb-8">Please authenticate to continue</p>

                <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                        <label className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 block flex items-center"><User size={12} className="mr-2" /> Username</label>
                        <input
                            type="text"
                            value={username}
                    ...
                    <div>
                        <label className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 block flex items-center"><Lock size={12} className="mr-2" /> Password</label>
                        <input
                            type="password"
                            value={password}
...
export const PasswordResetScreen: React.FC = () => {
    const { login, logout } = useHospital();
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-300 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-2xl max-w-md w-full">
                ...
                <h2 className="text-2xl font-black text-white text-center mb-2">Action Required</h2>
                <p className="text-slate-500 text-center text-sm mb-8">For security, you must reset your temporary password before proceeding.</p>

                <form onSubmit={handleReset} className="space-y-4">
                    <div>
                        <input type="password" placeholder="Current Password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-white outline-none focus:border-amber-500 transition-all" required />
                    </div>
                    <div>
                        <input type="password" placeholder="New Password" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-white outline-none focus:border-amber-500 transition-all" required />
                    </div>
                    <div>
                        <input type="password" placeholder="Confirm New Password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-white outline-none focus:border-amber-500 transition-all" required />
                    </div>
                    {error && <p className="text-rose-500 text-sm mt-2 font-bold">{error}</p>}
        ...
        <div className="max-w-2xl mx-auto space-y-8">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-xl">
                ...
                <div className="mb-8 p-4 bg-slate-800/50 rounded-xl border border-slate-700">
                    <p className="text-sm text-slate-300"><strong>Name:</strong> {currentUser?.name}</p>
                    <p className="text-sm text-slate-300"><strong>Role:</strong> {currentUser?.role}</p>
                </div>

                <h4 className="text-md font-bold text-white mb-4 flex items-center"><Lock size={16} className="mr-2" /> Change Password</h4>
                <form onSubmit={handleReset} className="space-y-4">
                    <div>
